...
- [x] D5 — pro 生图 `size` 可选 2K/4K ✅(2026-06-13,分支 `feat/pro-image-size-select`,详见 `feat-pro-image-size-select-brief.md`)— 仅 `gemini-3-pro-image-preview` 认客户传的 `size`(`2K`/`4K`/大小写不限,或 `2048x2048`/`4096x4096`),默认仍 4K(零行为变化);其余两个生图模型忽略 `size` 维持固定档(2.5→1K / 3.1→2K)。三入口都接:`/v1/chat/completions`(`body.size`)+ `/v1/images/{edits,generations}`(JSON `body.size` / multipart `form.get('size')`)。新增纯函数 `SIZE_SELECTABLE_MODELS` / `normalizeImageSize` / `resolveImageSize`(非 pro 或没传 → 固定档;pro 传无法识别值 → `ImageSizeError` → 400 invalid_request_error 不打上游),两处注入点共用。**唯一改动文件** `src/app/v1/[...path]/route.ts`(+ `__tests__/proxy.test.ts` 9 新测 + `/docs` size 注记),无 DB / 无 deps。**§4 实测计费(native `:generateContent` 直打 prod,throwaway user 量 quota delta,已清理)**:pro@4K 与 pro@2K **均扣 quota=71430 = ¥0.50/张(完全相等)**→ **new-api 按模型名计费,不按分辨率**,故 **pro 选 2K 不省钱**;flash@2K=37140=¥0.26/张。**客户引导**:本功能价值 = pro 画质 + 更小尺寸(更快下载/指定 2K),**不是省钱**;要便宜的 2K 直接用 `gemini-3.1-flash-image-preview`(¥0.20)。tsc/lint/prettier clean,`vitest src/app/v1` 67/67 + 全套 **190 files / 1794 pass / 1 skip / 0 fail**。**代码完成 + 全绿,PR 待 operator merge + 手动部署 + 真 key 冒烟(pro `size=2K`→ 打开图确认 2048²)。**
