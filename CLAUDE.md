...
- [x] D6 — **4K SKU ×2 上线(ch67 amutes 上游)** ✅(2026-07-27,PR #277 merge `8ab8d4e`)— 新别名 `gemini-3.1-flash-image-preview-4k`(¥0.35)+ `gemini-3-pro-image-preview-4k`(¥0.55),都锁 4K(size 无效)。proxy:两别名进 `GEMINI_IMAGE_MODELS`('4K')+ `GEMINI_ASPECT_RATIOS`(复用对应档);new-api(`scripts/configure-gemini-4k-skus.mjs`):`ModelPrice`=¥/1.2(0.291667/0.458333)+ MR/CR=0,ch#67 models+model_mapping(别名→真名)。计费实测(throwaway,prod):两 SKU 真出 **5632×3072** 4K 图,扣费分毫不差 ¥0.350/¥0.550。⚠️ 首轮 probe 撞出 default 组误翻倍事故(见下方计费语义节)。

### 计费语义(2026-07-20 GR 原生语义重锚 — 改价必读)

- **GroupRatio = 该组 ¥/$ 汇率直读**(new-api 分组倍率面板里的数就是那档卖多少 ¥ 换 $1)。现值:default/vip/svip 1.2、az-gpt 1.5、official-gpt 2.6、official 3.4、claude-aws-platform-t2 2.4、claude-aws-bedrock 2.7、cc-kiro 0.4、图片/视频组全部 1.2、**pool-gpt 0.2**(新组,承接原 default 里的 GPT 聊天池子,ch33/ch70;default key 调 gpt-5.x 已 503,受影响 28 客户已发 pool-gpt 档 key)、**pool-claude 2.4**(2026-07-27 新组,承接原 default 里的 Claude ccmax 号池 ch119/124/68/126;受影响 15 客户已发 pool-claude 档 key;portal `channel_groups` 已加行,`scripts/configure-pool-claude-group.mjs`。⚠️ 迁移当晚 operator 在 admin UI 并发调整号池渠道:新建 ch126「0.8ccmax asher」且把 ch119 加回 default — **ch119/126 若挂 default 组,default key 调 claude 就按 1.2 计,涨价意图落空**;新号池渠道 group 一律该写 `claude-aws-platform-t2,pool-claude`,不挂 default)。
- ⚠️ **2026-07-26~27 default 组误翻倍事故**:operator 想给 Claude 号池涨 2.4,在 UI 把 `GroupRatio.default` 整组拨到 2.4(07-26 16:29 UTC 起 default 全系 2× 多收,实测计费读的是 UI 键 `GroupRatio`,与平铺键 `group_ratio_setting.group_ratio`=1.2 脱钩)。07-27 以 pool-claude 拆组落地本意后,default 已拨回 1.2(两键同写 + 重启)。改整组 ¥ 率必须两键同写 + 重启,单改一键必翻车(同 az-gpt ¥5,593 事故)。
- per-model 基准(组无关):chat `ModelRatio = 官方$in ÷ 2`、`CompletionRatio = $out/$in`、`CacheRatio = $cache/$in`;图片/视频 `ModelPrice = ¥零售 ÷ 1.2`。实扣 ¥/1M = 2 × MR × GR(quota 标尺 500k=¥1)。
- **改价一律走 /admin/pricing**(PR #255 起 pricing-sync 组感知:换算自动 ÷ 该档组倍率,档次经 `channel_groups.newapi_group` → live GroupRatio 解析,解析失败拒同步)。改某档整体 ¥ 率 → 直接改 GroupRatio + `group_ratio_setting.group_ratio` 两处。**任何 2026-07-20 之前文档/脚本里的「GR=¥×5 / ÷1.2」换算全部作废。**
- 切换备份:`/root/newapi-pre-gr-rebase-20260720.dump`;迁移明细见 memory `project_gr_native_rebase_2026_07_20`。
...
### 企业门户「火山」渠道换上游 → service-inference.ai(2026-09-22,筷子合作终止)

- [x] 分支 `feat/enterprise-volc-svcinf`(worktree `silkroadai-volc-svcinf`)— volc region 上游从筷子换成 **service-inference.ai `/v2` doubao-sd-max 线**(`POST /v2/video/generate` / `GET /v2/video/tasks/{id}`,Bearer `sk-inf-v1-…`)。真金实测四档(pro/fast/mini/2.5)**全落真火山方舟**(`task.metadata` 即方舟原生任务体,`id: cgt-…`、成片 ark-acg-cn-beijing TOS 直链、token 数与筷子逐字一致 fast 480p 4s=40594)→ **fast/mini 恢复在售**(下架改 env `ENTERPRISE_VOLC_WITHDRAWN_MODELS`);fast/mini 仅 480p/720p(1080p 实测 400)。**proxy 主干 / 计费 / 对客契约 / region 键全不变**,差异全吸收在适配器边界。
    - **新 `src/lib/seedance/volc-adapter.ts` 取代 `kuaizi-adapter.ts`(已删)**:对客沿用「提交即自造 cgt- 形号 + volc_id_map 映射」(上游号 mvt-);轮询从 `task` + `task.metadata` 拼对客体(duration/ratio/resolution/seed/framespersecond/generate_audio/execution_expires_after/created_at/updated_at/last_frame_url);`preparing`(素材上传中)→ queued;失败 `task.error` 是字符串,`cleanTaskError` 剥 TOS 内部残渣(`tos: request error… RequestID=… EC=`);上游错误信封把方舟原错以**转义且截断**的 JSON 嵌在 message 里 → `unwrapUpstreamError` 取最内层 message/code 再交 `passthroughUpstreamError`。**resolution 上游必填**(文档说缺省 720p 是错的);**未知字段上游原样转方舟**(camera_fixed/service_tier 由方舟按任务类型判);无取消端点。契约守护 fixture 换成 2026-09-22 真实响应(含 metadata 内层键)。
    - **素材库回归平台库**(operator 拍板「我们自己做」):新上游只有传 URL 拿句柄 / 按 id 查,无 list/delete/组 → `kuaizi-assets.ts` + `volc-group-meta.ts` 删,route.ts 不再分流;volc 客户素材 = R2 + 行级归属,生成时 `asset://` 由 proxy 解析成 R2 直链发上游(上游 `preparing` ~4s)。**存量 11 万条筷子素材映射失效,客户需重传**(shujubao 基本每任务新建素材,影响小)。
    - **真人活体认证下架不删**(operator 拍板):`real-person.ts` 整体保留,route.ts 两个 VisualValidate Action 缺省返 `503 ServiceUnavailable`(暂停文案),`ENTERPRISE_REALPERSON_ENABLED=1` 恢复。新上游对含真人人脸素材直接拒(`PrivacyInformation`),无活体通道。
    - **价格**:上游按 USD/token 计价(`GET /v1/models` pricing 按 `<res>_<no_ref|with_ref>` 分桶,with_ref 仅 reference_video),pro/2.5 480p·720p 恰 = 我们挂牌 ¥ ÷ 6.70;7.2 汇率下 pro/2.5 主档成本高于挂牌,operator 自行谈价(费率表未动)。
    - env:`ENTERPRISE_VOLC_UPSTREAM_BASE_URL` / `ENTERPRISE_VOLC_UPSTREAM_KEY`(替代 `ENTERPRISE_KUAIZI_*`)+ 可选 `ENTERPRISE_VOLC_MODEL_{PRO,FAST,MINI,25}`(缺省方舟 id + `-max`)。无 migration、无新依赖。部署 = 只重建 3 个 `seedance-portal` 副本。

### 国内版 seedance-2-5 480p 单档换上游 → service-inference.ai /v1(2026-09-22)

- [x] 分支 `feat/cn-25-480p-svcinf` — 此前 480p 单档走 xinhankr `doubao-seedance-2-5-260628`(#446/#450),现改走 **service-inference.ai `/v1/video/generate` + `/v1/video/tasks/{id}`**(operator 指定 /v1;实测与 /v2 信封一致、直传 URL 参考图可用),上游模型 `doubao-seedance-2-5-260628-max`,**独立 key `SEEDANCE_SVCINF_KEY`**(与火山渠道的 `ENTERPRISE_VOLC_UPSTREAM_KEY` 不同一把)。720p/1080p 仍走 xinhankr pro 版 260801。
    - 抽出 **`src/lib/seedance/svcinf-client.ts`**(提交/轮询信封、`unwrapUpstreamError` 错误拆包、`task.error` 脱敏、`makeArkTaskId`),volc-adapter 与 cn-adapter 共用;volc-adapter 只剩 body 翻译 + 档位表。
    - cn-adapter:`SeedanceModelSpec` 加 `provider:'svcinf'` + `svcinfModel`,MODEL_MAP 的 `seedance2.5-480p[-ref]` 打标;`submitVideoWithKey` 在 xinhankr body 组好后翻成方舟 `content` 数组(prompt→text,images{url,role}→image_url,videos→video_url reference_video,audios→audio_url reference_audio),对客 id 自造 `cgt-` + `volc_id_map(kind='task')` 记 `mvt-`;`pollVideoWithKey` / `cancelVideoWithKey` 先查映射,mvt- → service-inference.ai(平台 key),否则存量 xinhankr 照旧(每次 cn 轮询 +1 次主键查表)。**未配 key 回落 xinhankr 260628**(部署缺 env 不断档)。
    - 零售(new-api ch95 → cn-proxy)与企业(proxy/reconcile)两条线都经 cn-adapter,自动生效;计费仍按 `usage.completion_tokens`。**部署面 = server1 seedance-portal×3 + portal + server2 portal-api-1..6 都要配 `SEEDANCE_SVCINF_KEY`。**

### 火山方舟形查询响应对齐官方 2026-09 新字段(2026-09-23)

- [x] 分支 `feat/ark-query-new-fields` — 官方「查询视频生成任务」新增 `execution_expires_after / frames / framespersecond / generate_audio / output_format / safety_identifier / seed / service_tier / tools`。此前**国内版只出旧文档 11 个字段**(测试钉死),火山渠道缺 `output_format / safety_identifier / frames`,`/api/v3` 提交白名单还把 `tools` 400 挡在门外。
...
- **QQ SMTP**: 邮件验证用(W3 启用)
- **service-inference.ai**(`model.service-inference.ai`,企业门户「火山」渠道视频上游,2026-09-22 起替代筷子): `/v2/video/generate` + `/v2/video/tasks/{id}`,四档 doubao-seedance 全落真火山方舟;平台级共享 key(`ENTERPRISE_VOLC_UPSTREAM_KEY`,仅企业实例需配)。素材库不接上游(无 list/delete 接口),volc 走平台库。零售海外满血线(ch41)也用它的 `/v1`。
- ~~**筷子 AI 开放平台**~~(2026-08-17 ~ 2026-09-22): 合作终止,视频/素材库适配器已删;真人活体认证代码保留但入口下架(`ENTERPRISE_REALPERSON_ENABLED` 缺省关)。
