/**
 * /api/portal/oss(W9 D3 PR-C)— 客户自定义 OSS 配置 CRUD。
 *
 * GET    — 读当前配置(secret 不返;access_key_id 返 mask)
 * PUT    — 保存配置:zod 校验 → 真连接测试(put+delete 临时对象)→
 *          通过才加密落库(status='active');测试失败 → 422 + message,不落库
 * DELETE — 清除配置,回平台默认 R2(幂等)
 *
 * Cookie-session auth(getCurrentUser);所有操作 WHERE user_id = 当前用户,
 * body 不收 id → 无 IDOR 面(镜像 balance-alert-threshold 的模式)。
 */
...
export async function PUT(req: NextRequest) {
    ...
    // 加密(也是 PORTAL_OSS_ENC_KEY 配置检查 — env 缺失这里 throw → 500 带 ops 信号)
    let encrypted: string;
    try {
        encrypted = encryptSecret(c.secret_access_key);
    } catch (e) {
        console.error('[oss] PORTAL_OSS_ENC_KEY missing/invalid', e);
