/**
 * /api/portal/oss/test-connection(W9 D3 PR-C)— 不持久化的连接测试。
 *
 * POST body 同 PUT /api/portal/oss 的 schema;put+delete 一个临时对象,
 * 返 { ok, message? }。秘钥只在内存中(加密 roundtrip 走同一 code path,
 * 顺带验证 PORTAL_OSS_ENC_KEY 在线)。
 */
